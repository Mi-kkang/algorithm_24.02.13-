    ...
    for j in range(i, len(garo)) :
        if garo[i] == garo[j] :             # 가로 시작점이 똑같을 때,
            if sero[i] == sero[j] :         # 세로 시작점도 같을 때, (=> 완전히 겹친다면)
                minus_n = 100
                rest = 100 - minus_n
                minus.append(minus_n)
                no_match.append(rest)

            elif sero[i] < sero[j] < sero[i] + 10 :
                minus_n = (sero[i] + 10 - sero[j]) * 10
                rest = 100 - minus_n
                minus.append(minus_n)
                no_match.append(rest)
            elif sero[j] < sero[i] < sero[j] + 10 :
                minus_n = (sero[j] + 10 - sero[i]) * 10
                rest = 100 - minus_n
                minus.append(minus_n)
                no_match.append(rest)
            ...
            if sero[i] == sero[j] :         # 세로 시작점은 같을 때,
                minus_n = (garo[i] + 10 - garo[j]) * 10
                rest = 100 - minus_n
                minus.append(minus_n)
                no_match.append(rest)
            elif sero[i] < sero[j] < sero[i] + 10 :
                minus_n = (garo[i] + 10 - garo[j]) * (sero[i] + 10 - sero[j])
                rest = 100 - minus_n
                minus.append(minus_n)
                no_match.append(rest)
            elif sero[j] < sero[i] < sero[j] + 10 :
                minus_n = (garo[i] + 10 - garo[j]) * (sero[j] + 10 - sero[i])
                rest = 100 - minus_n
                minus.append(minus_n)
                no_match.append(rest)
            ...
            if sero[i] == sero[j] :         # 세로 시작점은 같을 때,
                minus_n = (garo[j] + 10 - garo[i]) * 10
                rest = 100 - minus_n
                minus.append(minus_n)
                no_match.append(rest)
            elif sero[i] < sero[j] < sero[i] + 10 :
                minus_n = (garo[j] + 10 - garo[i]) * (sero[i] + 10 - sero[j])
                rest = 100 - minus_n
                minus.append(minus_n)
                no_match.append(rest)
            elif sero[j] < sero[i] < sero[j] + 10 :
                minus_n = (garo[j] + 10 - garo[i]) * (sero[j] + 10 - sero[i])
                rest = 100 - minus_n
                minus.append(minus_n)
                no_match.append(rest)
